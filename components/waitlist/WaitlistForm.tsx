...
}, { ssr: false }) as unknown as HCaptchaForwardRef;
...
export default function WaitlistForm({
  defaultEmail = '',
  source,
  headingId = 'waitlist-form-heading',
  className,
  onSuccess,
}: WaitlistFormProps) {
  ...
  return (
    <Card
      className={cn(
        'flex flex-col gap-xl px-gutter py-2xl sm:px-2xl sm:py-3xl',
        !prefersReducedMotion && 'transition-all duration-500 ease-out',
        !prefersReducedMotion && (isAnimated ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-4'),
        className
      )}
    ...
    >
      {!captchaEnabled && isDevLike ? (
        <div
          className="rounded-2xl border border-status-warning bg-status-warning/10 px-md py-sm text-body text-status-warning"
          role="status"
        >
