...
type PodiatristBadgeProps = {
  asSection?: boolean;
  cardClassName?: string;
};

export default function PodiatristBadge({ asSection = true, cardClassName }: PodiatristBadgeProps = {}) {
  ...
  const content = (
    <div
      className={cn(
        layout.card,
        shadows.soft,
        'flex flex-col gap-sm rounded-2xl border border-border bg-bg px-lg py-md md:flex-row md:items-center md:gap-md',
        cardClassName
      )}
    >
      ...
      <div className="flex flex-1 flex-col gap-xs">
        <h3 className="text-h3 text-text">{t('title')}</h3>
  ...
  return (
    <section className={cn(layout.section)}>
      <div className={cn(layout.container)}>{content}</div>
    </section>
  );
}
