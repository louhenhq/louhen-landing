...
- **Route shape:** All marketing routes live under `app/(site)/[locale]/…` using lowercase BCP-47 segments. The only prefixless document is the homepage (`/`); every other marketing or legal surface (Method, privacy, terms, imprint, etc.) is locale-prefixed. Prefixless pages that remain (home, legacy utility shells) continue to call `unstable_setRequestLocale(defaultLocale)` before rendering so metadata and hreflang stay aligned.
