...
- **2025-10-08 — CI/Playwright Server Lifecycle 2025**
  ...
  - Add → `start:test` enforces `NODE_ENV=production`, binds to `127.0.0.1:4311`, and captures output in `.next/test-server.log`; Playwright config pre-creates `playwright-report/` + `test-results/` and publishes JSON to `playwright-report/report.json`. Managed run now waits on `/status` for readiness, and a conditional fallback job pre-starts the server and reuses it when the managed path fails so CI still runs coverage.
